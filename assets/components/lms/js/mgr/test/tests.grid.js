LMS.grid.Tests = function(config) {
	config = config || {};
	...
	Ext.applyIf(config, {
		...
		standalone: false,
		stateful: true,
		stateId: 'lms-tests-state',
	});
	LMS.grid.Tests.superclass.constructor.call(this,config);
	this.getStore().sortInfo = {
		field: 'createdon',
		direction: 'DESC'
	};
};
Ext.extend(LMS.grid.Tests, MODx.grid.Grid, {
	...
	getColumns: function(config) {
		return [{
				...
				header: _('test_createdon'),
				dataIndex: 'createdon',
				width: 50,
				sortable: true,
				renderer: LMS.utils.formatDate,
			},{
				header: _('test_publishedon'),
				dataIndex: 'publishedon',
				width: 50,
				sortable: true,
				renderer: LMS.utils.formatDate,
			},{
				header: _('test_parent'),
				dataIndex: 'course',
				width: 75,
				sortable: true,
				renderer: function(value, metaData, record) {
	...
	createTest: function() {
		var createPage = MODx.action
			...
			: 'resource/create';
		location.href = 'index.php?a=' + createPage + '&class_key=Tests&parent=' + MODx.request.id + '&context_key=' + MODx.ctx;
	},
	...
	deleteTest: function(btn,e) {
		this.ticketAction('delete');
	},

	undeleteTest: function(btn,e) {
		this.ticketAction('undelete');
	},

	publishTest: function(btn,e) {
		this.ticketAction('publish');
	},

	unpublishTest: function(btn,e) {
...
Ext.reg('lms-grid-tests', LMS.grid.Tests);