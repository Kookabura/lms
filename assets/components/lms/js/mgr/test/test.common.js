LMS.panel.Test = function(config) {
	config = config || {};

	Ext.applyIf(config, {});
	LMS.panel.Test.superclass.constructor.call(this, config);
};

Ext.extend(LMS.panel.Test, MODx.panel.Resource, {
		...
		for (var i in originals) {
			...
			if (item.id == 'modx-resource-tabs') {
				...
				for (var i2 in item.items) {
					...
					// Hide permission tab because it handeled in background
	...
	getCheckboxes: function(config) {
		...
		var tmp = {
			...
			disable_jevix: {
				...
				checked: config.record.properties['lms']['disable_jevix']
			},
			cacheable: {},
			process_tags: {
				...
				checked: config.record.properties['lms']['process_tags']
			},
			published: {},
...
Ext.reg('modx-panel-test', LMS.panel.Test);