LMS.panel.Module = function(config) {
	config = config || {};

	Ext.applyIf(config, {});
	LMS.panel.Module.superclass.constructor.call(this, config);
};

Ext.extend(LMS.panel.Module, MODx.panel.Resource, {
	...
	getFields: function(config) {
		...
		for (var i in originals) {
			...
			if (item.id == 'modx-resource-tabs') {
				item.stateful = true;
				item.stateId = 'lms-' + config.mode + '-tabpanel';
				...
				for (var i2 in item.items) {
					...
					// Hide permission tab because it will be handeled in background
		...
		return [
			{
				...
				id: 'lms-box-publishing-information',
				defaults: {
					msgTarget: 'under'
				},
				items: [{
					...
					xtype: MODx.config.publish_document ? 'lms-combo-user' : 'hidden',
					...
					xtype: MODx.config.publish_document ? 'lms-combo-course' : 'hidden',
					id: 'lms-combo-course',
					...
					id: 'lms-item-file',
					fieldLabel: _('module_file_upload'),
					description: '<b>[[*file]]</b><br />',
					name: 'file',
					anchor: '100%',
					buttonText: _('lms_file_add'),
					value: config.record.properties['lms']['file']
				}, {
					xtype: 'hidden',
					name: 'menutitle',
	...
	getCheckboxes: function(config) {
		...
		var tmp = {
			...
			show_in_tree: {
				boxLabel: _('module_show_in_tree'),
				description: '<b>[[*show_in_tree]]</b><br/>' + _('module_show_in_tree_help')
			},
		};
...
Ext.reg('modx-panel-module', LMS.panel.Module);