LMS.panel.CourseSettings = function(config) {
	config = config || {};
	...
	Ext.applyIf(config, {
		...
		forceLayout: true,
		stateful: true,
		stateId: 'lms-course-settings-tabs',
		stateEvents: ['tabchange'],
		getState: function() {
			return {activeTab: this.items.indexOf(this.getActiveTab())};
		},
		items: this.getTabs(config),
		bodyCssClass: 'lms-course-settings ' + (MODx.modx23 ? 'modx23' : 'modx22')
	});
	LMS.panel.CourseSettings.superclass.constructor.call(this, config);
};
Ext.extend(LMS.panel.CourseSettings, MODx.VerticalTabs, {
	...
	getTabs: function(config) {
		...
		tabs.push({
			...
			items: this.getCourseSettings(config)
		});
	...
	getContentField: function(config) {
		return [{
			...
			value: config.record && config.record.content
				...
				: MODx.config['lms.course_content_default'],
	...
	getCourseSettings: function(config) {
		var items = [{
			html: _('lms_course_tab_lms_intro'),
			border: false,
			bodyCssClass: 'panel-desc',
		}];

		var tmp = {
			template: {xtype: 'lms-children-combo-template', anchor: '50%'},
		...
		for (var i in tmp) {
			if (tmp.hasOwnProperty(i)) {
				items.push(Ext.apply({
						...
						name: 'properties[lms][' + i + ']',
						hiddenName: 'properties[lms][' + i + ']',
						fieldLabel: _('lms_course_settings_' + i),
						id: 'lms-settings-children-' + i,
						value: config.record.properties['lms'][i],
						listeners: config.listeners,
						anchor: '25%'
					},
					tmp[i]
				...
				items.push({
					xtype: 'label',
					html: _('lms_course_settings_' + i + '_desc'),
					cls: 'desc-under'
				});
			}
		}
...
Ext.reg('lms-course-tab-settings', LMS.panel.CourseSettings);