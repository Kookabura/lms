LMS.grid.Professions = function(config) {
	config = config || {};

	Ext.applyIf(config, {
		url: LMS.config.connector_url,
		baseParams: {
			action: 'mgr/profession/getlist',
		},
		...
		stateful: true,
		stateId: 'lms-profession-state',
	});
	LMS.grid.Professions.superclass.constructor.call(this,config);
	this.getStore().sortInfo = {
		field: 'id',
		direction: 'DESC'
	};
};
Ext.extend(LMS.grid.Professions, MODx.grid.Grid, {
	...
	companyAction: function(method) {
		...
		MODx.Ajax.request({
			url: LMS.config.connector_url,
			params: {
				action: 'mgr/profession/multiple',
				method: method,
...
Ext.reg('lms-grid-profession', LMS.grid.Professions);