...
// Joining tables
$leftJoin = array(
	'Course' => array('class' => 'Course', 'on' => '`Course`.`id` = `Module`.`parent`'),
	'Statistic' => array('class' => 'Statistic', 'on' => '`Statistic`.`parent` = `Module`.`id` AND Statistic.user_id = '. $modx->user->get('id'))
);
...
$select = array(
	...
	'Module' => $modx->getSelectColumns($class, $class, '', array('content'), true),
);
...
if (!empty($rows) && is_array($rows)) {
	foreach ($rows as $k => $row) {
		...
		$row['class'] = empty($row['finished']) ? 'class="nom"' : 'class="yes"';
		$row['active'] = isset($row['finished']) ? ($row['finished'] == 0 ? 1 : 0) : 1;
