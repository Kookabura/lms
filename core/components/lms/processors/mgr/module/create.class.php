...
class ModuleCreateProcessor extends modResourceCreateProcessor {
	/** @var Module $object */
	public $object;
	...
	public function setFieldDefaults() {
		...
		if ($this->parentResource) {
			...
			$this->setProperty('resource_groups', $this->modx->toJSON($resource_groups));
		}
	...
	public function checkParentPermissions() {
		...
		if ($parentId > 0) {
			...
			if (!empty($courses) && !in_array($parentId, $courses)) {
				return $this->modx->lexicon('module_err_wrong_parent');
			}
			...
			if ($this->parentResource) {
				...
				elseif (!$this->parentResource->checkPolicy('course_add_children')) {
					return $this->modx->lexicon('module_err_wrong_parent');
				}
			}
			else {
			...
			return $this->modx->lexicon('module_err_access_denied');
		}
		return true;
	}
